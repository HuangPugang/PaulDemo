...
public class ScaleRecycleView extends RecyclerView {
    ...
    public ScaleRecycleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mPaint.setAntiAlias(true);
    }

    public ScaleRecycleView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        Log.i("TAG", "childCenterX=" + childCenterX + ",childCenterY="
                + childCenterX + "getHeight=" + getHeight() + ",left=" + left
                + ",top=" + top + ",r=" + r + ",absChildCenterY=" + absChildCenterY);
    ...
    private void prepareMatrix(final Matrix outMatrix, int distanceY, int r) {
        ...
        float z = r - translateZ * 1.1f;
        double radians = Math.acos((float) d / r);
        double degree = 90 - (180 / Math.PI) * radians;
        Log.e("TAG", "z=" + z);
        mCamera.save();
        // 三个参数分别代表x，y，z的坐标，随着时间，从（0,0,1300）向（0,0,0）
        // 变化，实现从远到近的效果。对于z轴（景深）变化，已经提供了透视的效果，无需scale设置。
//        if (z>200) {
//            mCamera.translate(0, 0, 700);
//        }
//        else if (z<-200){
//            mCamera.translate(0,0,-700);
//        }
//        else
    ...
    // calculate the highlight degree of elements
